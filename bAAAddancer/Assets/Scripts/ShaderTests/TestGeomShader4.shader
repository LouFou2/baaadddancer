Shader "TestGeomShader4"
{
    Properties
    {
        ...
        _EdgeLengthThreshold("Edge Length Threshold", Float) = 1
    }
    SubShader
    {
        HLSLINCLUDE
        ...
        struct GeomData
        {
            float4 positionCS             : SV_POSITION;        // Corresponds to positionCS in PackedVaryings
            float3 positionWS             : INTERP0;            // Corresponds to interp0 in PackedVaryings (stores positionWS)
            float3 normalWS               : INTERP1;            // Corresponds to interp1 in PackedVaryings (stores normalWS)
            float4 tangentWS              : INTERP2;            // Corresponds to interp2 in PackedVaryings (stores tangentWS)
            float3 viewDirectionWS        : INTERP3;            // Corresponds to interp3 in PackedVaryings (stores viewDirectionWS)
            float2 lightmapUV             : INTERP4;            // Corresponds to interp4 in PackedVaryings (stores staticLightmapUV or dynamicLightmapUV)
        ...
        void geom(triangle GeomData input[3], inout TriangleStream<GeomData> triStream)
        {
            ...
            float3 barycenter = (input[0].positionWS + input[1].positionWS + input[2].positionWS) / 3;
            float3 baryNormal = (input[0].normalWS + input[1].normalWS + input[2].normalWS) / 3;

            float edgeLength1 = length(input[1].positionWS - input[0].positionWS);
            float edgeLength2 = length(input[2].positionWS - input[1].positionWS);
            float edgeLength3 = length(input[0].positionWS - input[2].positionWS);

            if(edgeLength1 < _EdgeLengthThreshold && edgeLength2 < _EdgeLengthThreshold && edgeLength3 < _EdgeLengthThreshold )
            {
                for( int i = 0; i < 3; i++ )
                {
                    vert = input[i];
            ...
            else
            {
                for( int i = 0; i < 3; i++ )
                {
                    ...
                    //each vertex will create a new triangle:
                    float3 edge = input[nextIndex].positionWS - input[i].positionWS; // Edge from vert to next vert
                    float edgeLength = length(edge);
                    float3 edgeDir = normalize(edge);
                    float3 triangleNormal = vert.normalWS;

                    float3 prevEdge = input[i].positionWS - input[prevIndex].positionWS;
                    float prevEdgeLength = length(prevEdge);
                    float3 prevEdgeDir = normalize(prevEdge);
                    ...
                    GeomData newVert2 = vert; // Start with current vert's properties
                    ...
                    GeomData newVert3 = vert; // Start with current vert's properties
