Shader "SHAD_ScreenBender2"
{
    Properties
    {
        ...
        [HideInInspector]_ThumbPosL("ThumbPosL", Vector) = (0, 0, 0, 0)
        [HideInInspector]_ThumbPosR("ThumbPosR", Vector) = (0, 0, 0, 0)
        [HideInInspector]_Pow("Pow", Range(1, 16)) = 1
        [HideInInspector]_ThumbMagnitudeL("ThumbMagnitudeL", Float) = 0
        [HideInInspector]_ThumbMagnitudeR("ThumbMagnitudeR", Float) = 0
        ...
        // these are the geom shader vars
        _ThumbInL("ThumbInL", Vector) = (0, 0, 0, 0)
        _ThumbInR("ThumbInR", Vector) = (0, 0, 0, 0)
    ...
    SubShader
    {
        ...
        uniform float2 _ThumbInL;
        uniform float2 _ThumbInR;
        ...
        float CalculateShrinkAmount(float4 vertClipPosition)
        {
            // we use the clip space, so literally the x and y position of the vertex on the screen
            // we need to get Vector2 thumbstick L input, and thumbstick R input
            // the thumbsticks have a normalised x,y input between -1,-1 and 1,1
            // ok, so the closer the vert x,y is to the thumbstick x,y (also remember there are 2 thumstick vectors, so they will add), the more we shrink
            // we need to clamp shrink to 0-1
            // we also use the magnitudes of the thumbsticks to scale the input values

            float2 thumbL = float2(_ThumbInL.x, -_ThumbInL.y);
            float2 thumbR = float2(_ThumbInR.x, -_ThumbInR.y);

            float2 vertXY = vertClipPosition.xy / vertClipPosition.w;
            float2 deltaL = vertXY - thumbL;
            float2 deltaR = vertXY - thumbR;
        ...
        #pragma multi_compile _ DOTS_INSTANCING_ON
        #pragma vertex vert
        //#pragma geometry geom  // *** DONT PUT HERE, IT BREAKS
        #pragma fragment frag
